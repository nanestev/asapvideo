...
audio_fade_out_t = 4
audio_tracks = [("track1.wav", 11), ("track2.wav", 11), ("track3.wav", 13)]

def make_from_dir(dir):
    # add all image files in the folder as input
    inputs = OrderedDict([(f, None) for f in os.listdir(dir) if imghdr.what(f) != None])
    ...
    # select audio track
    audio_track = audio_tracks[random.randint(0, len(audio_tracks)-1)] 
    ...
    ff = FFmpeg(
        global_options = ["-y"],
        inputs = inputs,
        outputs = {"out.mp4": "-filter_complex \"" + ";".join(applied_filters) + "\" -map \"[video]\" -map \"[audio]\" -c:v libx264 -pix_fmt yuvj420p -q:v 1"}
	)
    #print ff.cmd
    ff.run()
    return True
